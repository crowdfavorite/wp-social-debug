		...
		if (isset($_GET['post_id'])) {
			...
			if (is_array($post) and isset($post[0])) {
				...
				echo 'INSERT INTO '.$wpdb->posts.'(ID, post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count)'.
					" VALUES('$post->ID', '".htmlentities($post->post_author)."', '$post->post_date', '$post->post_date_gmt', '".htmlentities($post->post_content)."', '".htmlentities($post->post_title)."', '".htmlentities($post->post_excerpt)."', '".htmlentities($post->post_status)."', '".htmlentities($post->comment_status)."', '".htmlentities($post->post_status)."', '$post->post_password', '".htmlentities($post->post_name)."', '$post->to_ping', '$post->pinged', '$post->post_modified', '$post->post_modified_gmt', '".htmlentities($post->post_content_filtered)."', '$post->post_parent', '".htmlentities($post->post_guid)."', '$post->menu_order', '".htmlentities($post->post_type)."', '".htmlentities($post->post_mime_type)."', '$post->comment_count');<br />";
				...
				foreach ($results as $result) {
					echo 'INSERT INTO '.$wpdb->postmeta.'(post_id, meta_key, meta_value)'." VALUES('$result->post_id', '".htmlentities($result->meta_key)."', '".htmlentities($result->meta_value)."');<br />";
				}
				...
				foreach ($results as $result) {
					echo 'INSERT INTO '.$wpdb->commentmeta.'(comment_id, meta_key, meta_value)'." VALUES('$result->comment_id', '".htmlentities($result->meta_key)."', '".htmlentities($result->meta_value)."');<br />";
				}
				echo '</pre>';
			}
		}
